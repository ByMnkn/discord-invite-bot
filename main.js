...
const { botOwner, botPrefix, botToken, guildID, botVoiceChannelID, inviteChannelID, status, play } = require('./config.json');
const guildInvites = new Map();
const color = require('paint-console')
const mongoose = require('mongoose');
mongoose.connect('mongodb+srv://dbUser:XXXXXXXXX@cluster0.gntcy.mongodb.net/myFirstDatabase?retryWrites=true&w=majority', { //mondo db info here
...
client.on("ready", async () => {
  ...
  if (botVoiceChannel) botVoiceChannel.join().catch(err => console.warn("Bot ses kanalına bağlanamadı!"));
    ...
    message.channel.send(embed)
      .then(() => {
        message.channel.awaitMessages(response => response.content === "evet", {
          ...
          .then((collected) => {
            message.channel.send(`**${client.user.username}** Bot yeniden başlatılıyor...`).then(message => {
              console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Bot yeniden başlatılıyor...`)
              process.exit(1);
            }).catch(console.error)
          })
          .catch(() => {
            message.channel.send('Yeniden başlatma işlemi iptal edildi.').then(x => x.delete({
              timeout: 30000
            }));
          });
      });
...
client.on("ready", async () =>
{
  ...
  console.error(`• Bot Başlatılıyor...`);
  console.info(`• Komutlar Başarıyla Yüklendi!`);
  console.info(`• Discorda Başarıyla Bağlandı!`);
  client.user.setStatus("online"); /// ("") kısmı (online - dnd - idle) gibi değiştirilebilir.
  client.user.setActivity("MNKN | bymnkn.com", {
    type: "PLAYING"
  }); //// type kısmı (WATCHING - PLAYING - LISTENING) gibi değiştirilebilir.
  console.info(`• play Başarıyla Ayarlandı!`);
  console.info(
    `• ` +
...
class Spammer extends yenile {
  ...
  start(timeout) {
    if(isNaN(timeout)) return console.error("Bot ping gönderildi!")
    setInterval(() => {
       console.log(this.startContent())
    } , parseInt(timeout))
  }
}

let spam = new Spammer(this)
let milisaniye = "1800000" 
spam.start(milisaniye);
